...
def send_message(self, subject, message, fromUser, toUser):
    ...
    if userNotificationQuery is not None:
        if userNotificationQuery.emailMessage is True:
            ...
            fullSiteURL = (
                sysSettings.siteProtocol + sysSettings.siteAddress + "/messages"
            )
            emailContent = (
                """
            <div>
                ...
                <ul>
                  <li><b>From: </b>"""
                + templateFilters.get_userName(fromUser)
                + """</li>
                  <li><b>Time Sent: </b>"""
                ...
                To view the full message, visit <a href='"""
                + fullSiteURL
                + """'>"""
                + fullSiteURL
                + """</a>
            ...
            send_email.delay(
                sysSettings.siteName + " - New Message Notification",
                userNotificationQuery.email,
                emailContent,
            )
    log.info(
        {
    ...
    for queue in webhookQueue:
        if queue:
            for hook in queue:
                ...
                payload = webhookFunc.processWebhookVariables(
                    hook.requestPayload, **kwargs
                )
...
def test_webhook(self, webhookType, webhookID, **kwargs):
    ...
    if webhookQuery is not None:
        ...
        payload = webhookFunc.processWebhookVariables(
            webhookQuery.requestPayload, **kwargs
        )
